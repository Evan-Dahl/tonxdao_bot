...
class Tapper:
    ...
    def save_user_agent(self):
        ...
        if not any(session['session_name'] == self.session_name for session in self.session_ug_dict):
            ...
            self.success(f"<light-yellow>{self.session_name}</light-yellow> | User agent saved successfully")
    ...
    async def get_tg_web_data(self, proxy: str | None) -> str:
        ...
        try:
            ...
            try:
                ...
                self.error(f"Session {self.session_name}: Peer not found - {e}")
                return None
            except ValueError as e:
                self.error(f"Session {self.session_name}: Invalid peer ID - {e}")
                return None
            ...
            self.info(f"Session {self.session_name}: Successfully retrieved tgWebAppData.")
            return tg_web_data

        except InvalidSession as error:
            self.error(f"Session {self.session_name}: Invalid session - {error}")
            raise error

        except Exception as error:
            self.error(f"Session {self.session_name}: Unknown error during Authorization - {error}")
    ...
    async def register_with_new_name(self, http_client: aiohttp.ClientSession, initdata, referral_token):
        ...
        while True:
            ...
            if resp_json.get("access_token"):
                ...
                self.info('Username taken, retrying register with new name')
    ...
    async def login(self, http_client: aiohttp.ClientSession, proxy: str | None) -> None:
        try:
            ...
            if not initdata:
                self.error("Failed to retrieve tgWebAppData. Cannot proceed with login.")
                return None
            ...
            while retry_count < max_retries:
                try:
                    ...
                    if resp_json.get("message") == "rpc error: code = AlreadyExists desc = Username is not available":
                        ...
                        self.token = resp_json.get("access_token")
                        self.success(f"Logged in successfully with username - {self.username}")
                        return self.token

                except aiohttp.ClientError as e:
                    self.error(f"HTTP Client error during login attempt {retry_count + 1}: {e}")
                except Exception as e:
                    self.error(f"Unexpected error during login attempt {retry_count + 1}: {e}")
                
                retry_count += 1
                delay = min(retry_delay * (2 ** retry_count), 60) 
                self.info(f"Retrying login in {delay} seconds... (Attempt {retry_count}/{max_retries})")
                await asyncio.sleep(delay)

            self.error("Max login attempts reached. Could not log in.")
            return None

        except Exception as error:
            self.error(f"Unexpected login error for {self.session_name}: {error}")
            return None
    ...
    async def get_info(self, http_client: aiohttp.ClientSession):
        ...
        if not self.token:
            self.error("Access token is missing, cannot fetch profile info.")
            return None, 0, 0, 'Unknown'
        
        try:
            self.info("Fetching profile.")
            ...
            if profile_data:
                ...
                self.success(f"Profile data initialized for {self.fullname} | Coins: {self.coins}, DAO Coins: {self.dao_coins}")
                return self.fullname, self.coins, self.dao_coins
            else:
                self.warning("Failed to fetch profile info.")
                return None, 0, 0, 'Unknown'
        
        except aiohttp.ClientError as e:
            self.error(f"Error fetching profile info: {e}")
            return None, 0, 0, 'Unknown'

        except Exception as e:
            self.error(f"Unexpected error fetching profile info: {e}")
            return None, 0, 0, 'Unknown'
        ...
        try:
            async with http_client.get(url=url, headers=auth_headers) as response:
                if response.status == 200:
                    ...
                    self.warning(f"Failed to fetch profile info. Status code: {response.status}")
                    return None
        except Exception as e:
            self.error(f"Unexpected error in get_profile_info: {e}")
            return None
    ...
    async def get_token(self, http_client: aiohttp.ClientSession):
        ...
        async with http_client.get(url=url, headers=auth_headers) as response:
            ...
            self.info(f"cref token is: {data}")
        ...
        try:
            ...
            async with http_client.get(url=url, headers=auth_headers) as response:
                if response.status == 200 and response.content_type == 'application/json':
                    ...
                    self.error(f"Unexpected response: {response.status}, content type: {response.content_type}")
                    return None
        except aiohttp.ClientError as e:
            self.error(f"Error fetching daily task info: {e}")
            return None
    ...
    async def get_daily_claim(self, http_client: aiohttp.ClientSession):
        ...
        try:
            ...
            self.error(f"Error claiming daily task: {e}")
            return None
    ...
    async def start_task(self, task_id: str, http_client: aiohttp.ClientSession):
        ...
        try:
            ...
            self.error(f"Error starting task {task_id}: {e}")
            return None
    ...
    async def claim_task(self, task_id: str, http_client: aiohttp.ClientSession):
        ...
        try:
            ...
            self.error(f"Error claiming task {task_id}: {e}")
            return None
    ...
    def apply_changes(self, msg):
        ...
        self.info(f"⚡ {self.session_name} Updated from RPC: | 🪙 Energy: {self.energy} | 💰 Coins: {self.coins} | 📈 Profit (DAO Coins): {self.profit}")
    ...
    async def check_proxy(self, http_client: aiohttp.ClientSession, proxy: Proxy) -> None:
        try:
            ...
            self.info(f"<light-yellow>{self.session_name}</light-yellow> | Proxy IP: {ip}")
        except Exception as error:
            self.error(f"<light-yellow>{self.session_name}</light-yellow> | Proxy: {proxy} | Error: {error}")
        ...
        while retry_count < max_retries:
            try: 
                if not self.c_token:
                    ...
                    if not self.c_token:
                        self.error("Failed to retrieve the required token, cannot proceed with WebSocket connection.")
                        return False
                        ...
                        try:
                            ...
                            for i, response in enumerate(responses, 1):
                                if response.strip() == "{}":
                                    ...
                                    self.info("Received ping message from server, responding with {}.")
                                    continue
                                
                                self.info(f"Response after click message ({i}): {response}")

                                try:
                                    response_data = json.loads(response)
                                    if "error" in response_data and response_data["error"].get("message") == "internal server error":
                                        self.error("Internal server error detected. Reconnecting and refreshing token.")
                                        await asyncio.sleep(retry_delay)
                                        retry_count += 1
                                        break 
                                    ...
                                    self.error(f"Failed to parse response: {response}, Error: {e}")
                            ...
                            for _ in range(self.NUMBER_OF_DISPLAY_MESSAGE):
                                ...
                                self.info(f"Sync response: {response}")

                                if response.strip() == "{}":
                                    await websocket.send("{}")
                                    self.info("Received empty message from server, responding with {}.")
                                    continue
                                ...
                                try:
                                    ...
                                    self.error(f"Failed to parse sync response: {response}, Error: {e}")

                                if self.energy <= 5:
                                    self.info("Energy depleted. Mining process completed.")
                                    return False
                            
                        except websockets.exceptions.ConnectionClosed as e:
                            self.error(f"Connection closed with code {e.code}: {e.reason}")
                            self.error(f"Attempting to reconnect and retrieve a new c_token after {retry_delay} seconds.")
                            await asyncio.sleep(retry_delay)
                            retry_count += 1
                            break 

            except Exception as e:
                self.error(f"Unexpected error during mining: {e}")
                return False

        self.error("Max retries reached. Exiting the mining process.")
        return False
    ...
    async def run(self, proxy: str | None) -> None:
        ...
        if settings.USE_RANDOM_DELAY_IN_RUN:
            ...
            self.info(f"Bot will start after {random_delay} seconds.")
        ...
        while retry_count < max_retries:
            try:
                ...
                if login_need:
                    ...
                    if not self.token:
                        self.error("Failed to log in, retrying.")
                        retry_count += 1
                ...
                if not full_name:
                    self.error("Failed to retrieve profile information, retrying login.")
                    login_need = True
                    retry_count += 1
                    await asyncio.sleep(retry_seconds)
                    continue

                self.success(f"User {full_name} | Coins: {coins}, DAO Coins: {dao_coins}")
                ...
                if tasks:
                    ...
                    if not inactive_tasks and not claimable_tasks:
                        self.info("No inactive tasks available to start and no claimable tasks.")
                    else:
                        for task in claimable_tasks:
                            task_id = task['id']
                            task_name = task['name']
                            self.info(f"Claiming task {task_name} (ID: {task_id}) that is ready to be claimed.")

                            try:
                                claim_response = await self.claim_task(task_id, http_client)
                                if claim_response:
                                    self.success(f"Task {task_name} claimed successfully.")
                                else:
                                    self.warning(f"Failed to claim task {task_name}. Moving to next task.")
                                    continue
                            except Exception as e:
                                self.error(f"Error while claiming task {task_name} (ID: {task_id}): {e}")
                                continue

                        for task in inactive_tasks:
                            task_id = task['id']
                            task_name = task['name']
                            self.info(f"Starting task {task_name} (ID: {task_id})")

                            try:
                                start_response = await self.start_task(task_id, http_client)
                                if start_response:
                                    wait = 5
                                    self.success(f"Task {task_name} started successfully. Waiting {wait} seconds before claiming.")
                                    await asyncio.sleep(wait)
                                else:
                                    self.warning(f"Failed to start task {task_name}. Moving to next task.")
                                    continue
                                ...
                                self.error(f"Error while handling task {task_name} (ID: {task_id}): {e}")
                                continue

                else:
                    self.info("No tasks available to process. Skipping task stage.")

                daily_info = await self.get_daily_info(http_client)

                if daily_info:
                    if daily_info.get("is_available", False):
                        self.success("Daily task available.")
                        daily_claim = await self.get_daily_claim(http_client)
                        if daily_claim and daily_claim.get('success', False):
                            self.success(f"Daily task claimed successfully. Reward: {daily_info.get('reward', 0)} coins.")
                        else:
                            self.warning(f"Failed to claim daily task. Response: {daily_claim}")
                    else:
                        ...
                        if last_claimed_at:
                            ...
                            if time_until_next_claim > 0:
                                ...
                                self.info(f"Daily task not available. Time left until next claim: {int(hours)} hours and {int(minutes)} minutes.")
                            else:
                                self.info("Daily task not available, but cooldown period has passed.")
                        else:
                            self.info("No daily task available.")
                else:
                    self.error(f"Unexpected daily task response: {daily_info}")

                mining_success = await self.mine(http_client, dt_string) 
                if not mining_success:
                    self.info("Mining process completed or no sufficient energy left.")
                    break


            except KeyboardInterrupt:
                self.warning("Mining process interrupted by the user.")
                break
                ...
                if retry_count >= max_retries:
                    self.error("Max retries reached. Exiting process.")
                    break
                else:
                    self.info(f"Retrying ({retry_count}/{max_retries})...")
